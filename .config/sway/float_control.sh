...
# Check if the script is already running using pgrep
if pgrep -x yad > /dev/null; then
    echo "The script is already running. Exiting..."
    exit 1  # Exit the script if it's already running
fi
...
get_current_brightness() {
    current_brightness=$(brightnessctl get | awk '{print int($1/7500*100)}')
...
result=$(yad --title="System Controls" \
             ...
             --separator=" " \
             --field=" :SCL" $current_volume \
             --field="🌗:SCL" $current_brightness \
             --field="Reboot:FBTN" "systemctl reboot" \
             --field="Log Off:FBTN" "bash -c 'waydroid session stop && sway exit'" \
             --field="Sleep:FBTN" "'pkill yad' && systemctl suspend" )
